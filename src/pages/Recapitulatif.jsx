...
export default function Recapitulatif({
  ...
  const villeSelectionnee = useMemo(() => {
    ...
    return (
      listeVilles.find((ville) => ville.nom === selection?.villeNom) || null
    );
  }, [selection]);

  const nombrePersonnes = Number(selection?.nbPersonnes) || 1;
  const nombreDeNuits = Number(selection?.nbNuits) || 1;
  ...
  const prixTotal = useMemo(() => {
    ...
    if (hebergementSelectionne) {
      sommeTotale +=
        (Number(hebergementSelectionne.prix) || 0) * nombreDeNuits;
    }
          ...
          {!hebergementSelectionne ? (
            ...
            <div className="ligne">
              <div>
                ...
                <div className="meta">
                  {hebergementSelectionne.categorie} •{" "}
                  {hebergementSelectionne.prix} $ / nuit • {nombreDeNuits} nuit(s)
                </div>
              </div>
